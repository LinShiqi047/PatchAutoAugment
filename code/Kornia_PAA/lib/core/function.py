...
def train(config, train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if config.AUG == 'PAA':
            # patch autoaugment
            if config.RLPROB == True:
                operations, probabilities, operation_logprob, operation_entropy, prob_logprob, prob_entropy, state_value = \
                    rl_choose_action(input, model_a2c)
                input_aug = patch_kornia(input, operations, 
                    config.TRAIN.BATCH_SIZE_PER_GPU*len(gpus), size=config.N_GRID, 
                    patch_size=config.MODEL.IMAGE_SIZE[0]//config.N_GRID,
                    epochs=config.TRAIN.END_EPOCH, epoch=epoch, 
                ...
                input_aug = patch_kornia(input, operations, 
                    ...
                    patch_size=config.MODEL.IMAGE_SIZE[0]//config.N_GRID,
                    epochs=config.TRAIN.END_EPOCH, epoch=epoch)
            if(input_aug.shape!=torch.Size([32, 3, 224, 224])):
                continue
            # compute output
            ...
            if config.RLPROB == True:
                loss_a2c = a2closs(operation_logprob, operation_entropy, state_value, reward, 
                                    prob_logprob=prob_logprob, prob_entropy=prob_entropy)
            else:
                loss_a2c = a2closs(operation_logprob, operation_entropy, state_value, reward)
            ...
            losses_a2c.update(loss_a2c.item(), input.size(0))
        ...
        if i % config.PRINT_FREQ == 0:
            if config.AUG == 'PAA':
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                    ...
                    'Accuracy@5 {top5.val:.3f} ({top5.avg:.3f})\t' \
                    'Loss_A2C {loss_a2c.val:.5f} ({loss_a2c.avg:.5f})\t'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      speed=input.size(0)/batch_time.val,
                      data_time=data_time, loss=losses, top1=top1, top5=top5, loss_a2c=losses_a2c)
            else:
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                    'Time {batch_time.val:.3f}s ({batch_time.avg:.3f}s)\t' \
                    'Speed {speed:.1f} samples/s\t' \
