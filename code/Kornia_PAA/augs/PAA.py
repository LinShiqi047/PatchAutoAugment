...
patch_ops = [
    ...
    RandomPosterize(bits=3, same_on_batch=False),
    RandomErasing(p=random.random(), scale=(0.09, 0.36), ratio=(0.5, 1/0.5), same_on_batch=False),
    #K.RandomSharpness(1.),
    RandomAffine(degrees=0., shear=(30, 30), same_on_batch=False),
    RandomAffine(degrees=0., shear=(30, 30), same_on_batch=False),
    RandomAffine(degrees=0., translate=(0.4, 0.4), same_on_batch=False),
    ColorJitter(brightness=(0.5, 0.95), same_on_batch=False),
    ColorJitter(contrast=(0.5, 0.95), same_on_batch=False),
    ColorJitter(contrast=(0.5, 0.95), same_on_batch=False),
    RandomRotation(degrees=30.0, same_on_batch=False),
...
def patch_kornia(image, operations, batch_size, size=2, patch_size=16, epochs=None, epoch=None, probabilities=None):
    
    print(image.shape)
    patch_aug_list = []
    for j in range(size):
        for k in range(size):
            operation = operations[0][j,k].item()
            patch_aug_list.append(patch_ops[operation])
    
    seq=K.container.patch.PatchSequential(
            patch_aug_list[0],
            patch_aug_list[1],
            patch_aug_list[2],
            patch_aug_list[3],
