...
import kornia.augmentation.container.patch as P
...
patch_ops = [
    RandomErasing(p=random.random(), scale=(0.09, 0.36), ratio=(0.5, 1/0.5), same_on_batch=False),
    RandomPosterize(bits=3, p=random.random(),same_on_batch=False),
    #RandomErasing(p=random.random(), scale=(0.09, 0.36), ratio=(0.5, 1/0.5), same_on_batch=False),
    RandomAffine(degrees=0., p=random.random(),shear=(30, 30), same_on_batch=False),
    RandomAffine(degrees=0., p=random.random(),shear=(30, 30), same_on_batch=False),
    RandomAffine(degrees=0., p=random.random(),translate=(0.4, 0.4), same_on_batch=False),
    ColorJitter(brightness=(0.5, 0.95), p=random.random(),same_on_batch=False),
    ColorJitter(contrast=(0.5, 0.95), p=random.random(),same_on_batch=False),
    RandomRotation(degrees=30.0, p=random.random(),same_on_batch=False),
    RandomGrayscale(p=random.random(), same_on_batch=False),
    ColorJitter(hue=(-0.3, 0.3),p=random.random()),
    ColorJitter(hue=(-0.3, 0.3),p=random.random()),
    RandomEqualize(p=random.random()),
    RandomSharpness(1.,p=random.random())
    
]

name_ops = [ "RandomErasing",
            "RandomPosterize",
            "RandomAffine",
            "ColorJitter",
            "RandomMixUp",
...
def patch_kornia(image, target, operations, batch_size, size=2, patch_size=16, epochs=None, epoch=None, probabilities=None):
    PS=P.PatchSequential(
    K.RandomErasing(p=random.random(), scale=(0.09, 0.36), ratio=(0.5, 1/0.5), same_on_batch=False),
    K.RandomPosterize(bits=3, p=random.random(),same_on_batch=False),
    K.RandomErasing(p=random.random(), scale=(0.09, 0.36), ratio=(0.5, 1/0.5), same_on_batch=False),
    K.RandomAffine(degrees=0., p=random.random(),shear=(30, 30), same_on_batch=False),
    K.RandomAffine(degrees=0., p=random.random(),shear=(30, 30), same_on_batch=False),
    K.RandomAffine(degrees=0., p=random.random(),translate=(0.4, 0.4), same_on_batch=False),
    K.ColorJitter(brightness=(0.5, 0.95), p=random.random(),same_on_batch=False),
    K.ColorJitter(contrast=(0.5, 0.95), p=random.random(),same_on_batch=False),
    K.RandomRotation(degrees=30.0, p=random.random(),same_on_batch=False),
    K.RandomGrayscale(p=random.random(), same_on_batch=False),
    K.ColorJitter(hue=(-0.3, 0.3),p=random.random()),
    K.ColorJitter(hue=(-0.3, 0.3),p=random.random()),
    K.RandomEqualize(p=random.random()),
    K.RandomSharpness(1.,p=random.random()),
    K.RandomEqualize(p=random.random()),
    K.RandomSharpness(1.,p=random.random()),
    grid_size=(size,size))
    ...
    for i in range(image.size(0)):
        list_ops = []
        param_list = []
        for num in range(number_ops):
            list_ops.append([])
        for j in range(size):
            for k in range(size):
                operation = operations[i,j,k].item()
                list_ops[operation].append(j*size+k)
        for num in range(number_ops):
            if(list_ops[num]!=[]):
                param_list.append(P.PatchParamItem(indices=list_ops[num],param=P.ParamItem(name=name_ops[num],data=None)))
            
        image_=torch.unsqueeze(image[i],0)
        target_ = torch.unsqueeze(target[i],0)
        # print(image_.size())
        # print(target[i].size())
        image_,target_=PS.forward(image_,target_,param_list)
        image[i]=image_.squeeze()
        target[i]=target_.squeeze()
...
#         for num in range(number_ops):
#             if(list_ops[i]!=[]):
#                 param_list.append(P.PatchParamItem(indices=list_ops[i],param=P.ParamItem(name=name_ops[i],data=None)))
            
#         image[i]=P.PatchSequential.forward(image[i],target[i],param_list)
